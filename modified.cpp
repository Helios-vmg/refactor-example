...
#pragma omp parallel for collapse(2)
 ...
 // convert to Fourier space
	...
	char neinitial[] = "ne_initial.txt";
	char Teinitial[] = "Te_initial.txt";
	char Tiinitial[] = "Ti_initial.txt";
	char phiinitial[] = "phi_initial.txt";
//print out size of x grid 
	char XXgrid[] = "X.txt";
	char YYgrid[] = "Y.txt";
	...
	print2DArrf(phiinitial, phi); //print2DArrf()
	...
	for (int iter = 0; iter < iter_max; iter++){
	 ...
	 for (int stage = 0; stage < 4; stage++){
		...
		if ((iter/saveFrequency) - saveNum == 0){
			...
			//snprintf(nefilename, 16, "%d", saveNum);

			//print2DArrf(Nfilename, std::string("N") + std::to_string(saveNumber) + ".txt");
			//instead of strcat: sprintf(str, "%s", other_str)
			//strcat(nefilename, type); //this was commented
			...
			print2DB(Tefilename, Te); //test
			print2DB(Tifilename, Ti); //test
			print2DB(phifilename, phi); //test
			print2DB(nefilename, ne); //test
...
void calcV_ExBk(double dphidxk[][ncomp], double dphidyk[][ncomp], double B[], double B2, double vexbkx[][ncomp], double vexbky[][ncomp]){ // Your inputs should be dphidxk, dphidyk, B, and B2
		#pragma omp parallel for collapse(3)
		for (int i = 0; i < nx; i++){
			for (int j = 0; j < nyk; j++){
				for (int k = 0; k < ncomp; k++){
